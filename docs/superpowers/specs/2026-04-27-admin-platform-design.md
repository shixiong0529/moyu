...
**管理员引导**：服务启动时读取 `.env` 的 `ADMIN_USERNAME`，自动将该用户 `is_admin` 置为 `True`。登录时额外校验 `is_banned`，被封禁用户返回 `403`。
...
| `target_id` | Integer | 被举报对象 ID |
| `reason` | String | 举报原因 |
| `status` | String | `pending` / `resolved` / `dismissed` |
| `resolved_by` | FK → users (nullable) | 处理管理员 |
...
| `target_type` | String | `user` / `server` / `channel` / `channel_group` / `message` / `report` / `invite` |
...
所有 `/api/admin/*` 路由在现有 JWT 校验基础上额外检查 `current_user.is_admin`，否则返回 `403 Forbidden`。
...
| POST | `/api/admin/users/{id}/ban` | 封禁用户，body: `{ reason }` |
| POST | `/api/admin/users/{id}/unban` | 解封用户 |
| PATCH | `/api/admin/users/{id}/admin` | 提升/撤销管理员，body: `{ is_admin: bool }` |
...
| DELETE | `/api/admin/channel-groups/{id}` | 强制删除分组（含其下所有频道） |
...
| GET | `/api/admin/reports/{id}` | 举报详情（含被举报内容快照） |
| POST | `/api/admin/reports/{id}/resolve` | 处理举报，body: `{ note }` |
| POST | `/api/admin/reports/{id}/dismiss` | 驳回举报，body: `{ note }` |
...
| 方法 | 路径 | 说明 |
|---|---|---|
| GET | `/api/admin/join-requests` | 全站待审加入申请列表（分页、按服务器筛选） |
...
| 方法 | 路径 | 说明 |
|---|---|---|
| POST | `/api/reports` | 普通用户提交举报，body: `{ target_type, target_id, reason }` |
...
```
AdminApp
├── AdminLogin              ← 未登录时显示，调用 /api/auth/login
└── AdminShell              ← 已登录主框架
...
- 登录后 JWT 存入 `localStorage['hearth-admin-token']`，与主站 `hearth-token` 隔离
- 所有请求携带 `Authorization: Bearer <token>`
- `401` 响应时清除 token，跳回登录页
...
2. **`backend/routers/admin.py`** — 实现全部管理 API，注册到 `main.py`
3. **`.env` 引导逻辑** — `main.py` 启动事件中读取 `ADMIN_USERNAME` 自动提权
4. **登录封禁校验** — `auth.py` 登录接口新增 `is_banned` 检查
5. **`POST /api/reports`** — 普通用户举报接口（加到现有 channels 路由或单独文件）
